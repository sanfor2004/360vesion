...
- SQLite is the canonical local database. Keep `DATABASE_URL="file:./dev.db"` compatible with Prisma.
...
- Keep database serialization in `lib/store.ts` backward-compatible when practical.
...
- Do not commit `.env`, SQLite databases, generated uploads, `.next`, or deployment credentials.
...
For database-model changes, also run:
...
- Data saves to and reloads from SQLite.
